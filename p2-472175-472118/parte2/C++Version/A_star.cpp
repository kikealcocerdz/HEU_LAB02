...
std::vector<std::tuple<int, int>> A_star::adyacents(std::tuple<int, int> & coordinates) {
  ...
  for (auto & neighbor : neighbors) {
    ...
    if (r < map.data.size() && c < map.data[0].size()) { result.push_back({r, c}); }
  }
  /*
  std::cout << row << col << '\n';
...
int A_star::get_cost(char & node) {
  if (isdigit(node)) { return node - '0'; }
  if (node == 'X') { return 1410065407; }
  return 1;
}
...
int A_star::resolve_actions(char & type) {
  ...
  if (ambulance.current_path.n.empty() && ambulance.current_path.c.empty() &&
    ...
    if (ambulance.current_path.passengers == comp) {
      ...
      if (!check) { std::cout << "Error creating the file\n"; }
      return 1000;
    }
...
std::tuple<int, int> A_star::find_nearest_passenger(std::tuple<int, int> & pos) {
  ...
  for (auto & non_cont : ambulance.current_path.n) { all_patients.push_back(non_cont); }
  for (auto & cont : ambulance.current_path.c) { all_patients.push_back(cont); }
    ...
    for (auto & neighbor : adyacents(ambulance.current_path.coordinates)) {
      ...
      if (0 < new_cost && new_cost < 1410065407) {
        ...
        if (push) {
          ...
          int priority                               = new_cost - n_delivered + new_heuristic;
...
bool A_star::create_and_write_file(Ambulance & ambulance) {
  try {
    std::ofstream file("output22.txt");  // Open the file
    ...
    file << "Tiempo Total: "
         << "HAZ EL TIEMPO"
         << "\n";
    file << "Coste total: " << ambulance.current_path.cost_til_now << "\n";
    file << "Longitud del plan: " << ambulance.current_path.path.size() << "\n";
    file << "Nodos expandidos: " << expanded_counter << "\n";
